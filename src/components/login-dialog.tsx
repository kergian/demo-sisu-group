...
const LoginDialog = () => {
  ...
  const onSubmit = async (ev: LoginDialog) => {
    ...
    try {
      ...
      dialogId === 'createAccount' && (req = await api.createUser({ body }))
      dialogId === 'login' && (req = await api.loginUser({ body }))
  ...
  return (
    <UI.Modal open={!!dialogId} onClose={() => dialog.close(dialogId!)}>
      ...
      <form onSubmit={onSubmit}>
        ...
        <div css={styles.controls}>
          <UI.Button type='reset' onClick={() => dialog.close(dialogId!)}>
            Cancel
          </UI.Button>
